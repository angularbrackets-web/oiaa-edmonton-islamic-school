            ...
            <div className="max-w-7xl mx-auto px-6 py-4">
              <div className="flex items-center justify-center gap-4">
                ...
                >
                  ...
                  <span className="text-sm font-medium relative z-10">Gallery</span>
                </motion.button>
              </div>
            </div>
          </motion.div>
            ...
            >
              ...
              <div className="relative z-10 px-4 sm:px-6 lg:px-12 py-6 sm:py-8 lg:py-16 pb-16 sm:pb-8 flex flex-col h-full min-h-[400px] sm:min-h-[500px] lg:min-h-auto">
                ...
                <div className="flex-1">
                  {achievements.length > 0 && achievements[currentAchievement] && currentAchievement < achievements.length && (
                    ...
                    >
                      ...
                      {/* Navigation Controls */}
                      <motion.div
                        className="flex items-center gap-4 sm:gap-6 pt-8"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        transition={{ delay: 0.8 }}
                      >
                        {/* Achievement dots navigation - Larger on mobile */}
                        <div className="flex gap-2 sm:gap-2">
                          {achievements.map((_, index) => (
                            <motion.button
                              key={index}
                              onClick={() => goToAchievement(index)}
                              className={`w-4 h-4 sm:w-3 sm:h-3 rounded-full transition-all duration-300 hover:scale-125 focus:outline-none touch-manipulation ${
                                index === currentAchievement ? 'bg-terracotta-red scale-125' : 'bg-white/30 hover:bg-white/50'
                              }`}
                        ...
                        <div className="flex gap-2 sm:gap-3">
                          <motion.button
                            ...
                            className="bg-white/10 hover:bg-white/20 text-white p-4 sm:p-3 rounded-full transition-all duration-200 backdrop-blur-sm border border-white/20 touch-manipulation"
                ...
                <div className="flex flex-col gap-4 pt-8 border-t border-white/10 mt-8 relative z-30 w-full">
                  {/* Dynamic CTA Button - Only shows if configured for current achievement */}
                  {achievements[currentAchievement]?.ctaText && achievements[currentAchievement]?.ctaUrl && (
                    <div className="flex flex-col sm:flex-row gap-4">
                      {achievements[currentAchievement]?.ctaStyle === 'secondary' ? (
                        <motion.a
                          href={achievements[currentAchievement]?.ctaUrl}
                          className="relative border-2 sm:border border-white/60 sm:border-white/50 hover:bg-white hover:text-black text-white px-6 py-4 sm:py-3 rounded-xl sm:rounded-lg font-bold transition-all duration-300 overflow-hidden text-base sm:text-sm flex-1 min-h-[52px] sm:min-h-auto touch-manipulation backdrop-blur-sm text-center"
                          whileHover={{
                            scale: 1.02,
                        ...
                        >
                          <span className="relative z-10">{achievements[currentAchievement]?.ctaText}</span>
                          <motion.div
                            ...
                            animate={{ x: ["-100%", "200%"] }}
                            transition={{ duration: 3.5, repeat: Infinity, ease: "easeInOut", repeatDelay: 3.5 }}
                          />
                        </motion.a>
                      ) : (
                        <motion.a
                          href={achievements[currentAchievement]?.ctaUrl}
                          className="relative bg-terracotta-red hover:bg-terracotta-red-dark text-warm-white px-6 py-4 sm:py-3 rounded-xl sm:rounded-lg font-bold transition-all duration-300 shadow-lg overflow-hidden text-base sm:text-sm flex-1 min-h-[52px] sm:min-h-auto touch-manipulation text-center"
                          whileHover={{
                            scale: 1.02,
